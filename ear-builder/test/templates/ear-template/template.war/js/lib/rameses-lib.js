...
//notification works in conjunction with session class. This wont work without it.
var Session = new function() {
	...
	var poll = function() {
		var sid = self.getId();
		if( !sid ) {
			if(window.console) window.console.log("Session error. Session is lost. Please provide an onlostsession handler to remove this message");
			return;
		}
		var d = {};
		d.sessionid = sid;
				...
				success: function( data ) {
					if( data!=null && data.trim().length > 0  ) {
						if( data.startsWith("TOKEN")) {
							...
							if(window.console) window.console.log("please try to reconnect service. we need to reconnect now");
							token = null;
							poll();
						}
						else {
