/***
    ...
    RequiredValidator
    Opener
    Registry = for launching actions from plugins.
    WindowUtil
    ProxyService
...
/**
this is a new edition. If you want to place environment variables that you want carried
to the service, just place it here. 
*/
...
function DocOpener( id, params, qp ) {
	...
	this.load = function() {
		Bookmark.navigate( this.id, this.params );
	}
		...
		$.ajax( 
			{
				url: "poll",
				type: "POST",
				data: { sessionid: sessionid, tokenid: token, host:host },
				error: function( xhr ) { 
				...
				success: function( data ) {
					if( data!=null && data.trim().length > 0  ) {
						if( data.startsWith("TOKEN")) {
							...
							else alert('connection started...new token id '+token);
							poll();
						}
						else if(data == "-1") {
							alert("please try to reconnect service. we need to reconnect now");
							token = null;
							poll();
						}
						else {
...
/*
id is the session id generated from the server.
token is the id for a specific browser session.
the session must poll to check if the session is 
still alive after some time. In case a browser is
closed without logging out, the session must be destroyed.
The session ha 
*/
var Session = new function() {
	...
	//when initing a session must not yet exist
	this.init = function() {
		id = $.cookie("sessionid");
		if( id ) {
			if( this.errorHandler ) {
				this.errorHandler( "session-exists" );
			}
			else {
				alert('an error occured in Session.init. Message is displayed because no errorHandler was defined');
			}	
		}
	}
	...
	this.create = function( sessionId ) {
		...
		$.cookie("sessionid",id, { expires: date })
	}
	...
	this.getId = function() {
		if(!id) {
			id = $.cookie( "sessionid" );
			if(id) {
				Env.data.sessionid = id;
				//update session every one minute.
				setInterval( updateSession, 1000 * 60);
			}
			else {
				if(this.errorHandler) {
					this.errorHandler("session-not-exist");
				}
				else {
					alert('an error occured in Session.start. Message is displayed because no errorHandler was defined');
				}
			}
		}	
