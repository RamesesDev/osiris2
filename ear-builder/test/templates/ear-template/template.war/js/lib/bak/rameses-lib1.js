...
var Bookmark = new function() {
	...
	this.start = function() {
		...
		if(appName.indexOf("Microsoft")>=0 || appName.indexOf("Opera")>=0) {
			hashSupported = false;
		}	
		if(hashSupported) {	
			...
			if(!window.location.hash && InvokerUtil.invokers.length>0) {
				self.navigate( InvokerUtil.invokers[0].id );		
			}
			else {
				self.loadContent();
			}	
	...
	this.navigate = function( id, params, qryParams ) {
		var hash = id;
		if(params!=null) {
...
function DocOpener( id, params, qp ) {
	...
	this.load = function() {
		Bookmark.navigate( this.id, this.params, this.qryParams );
		/*
		var inv = InvokerUtil.find(this.id);
		if(inv==null)
			throw new Error("hash " + hash + " is not registered" );
		if( !inv.page) return;
		WindowUtil.load( inv.page, this.params, this.qryParams );
		*/
		/*
		if(this.caller.bookmark) {
			this.caller.bookmark.invoke( this.id,  this.params, this.qryParams );
		}
		*/
	}
...
function Notification(id, context) {
